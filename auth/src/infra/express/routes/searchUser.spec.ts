...
describe('SignInRouter', () => {
  ...
  it('should be able to search a authenticated user', async () => {
    ...
    const signinResponse = await request(app).post('/api/users/signin').send(data);
    const token = signinResponse.get('Set-Cookie');
    const searchResponse = await request(app).get(`/api/users?email=${data.email}`).set('Cookie', token);
